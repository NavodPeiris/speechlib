...
def test_detect_unknown_speakers_returns_empty_when_all_speakers_known(tmp_path):
    ...
    with (
        ...
        patch(
            "speechlib.speaker_recognition.speaker_recognition", return_value="Agustin"
        ),
    ):
...
def test_detect_unknown_speakers_multiple_unknowns(tmp_path):
    ...
    mock_pipeline = _make_diarization_mock(["SPEAKER_00", "SPEAKER_01", "SPEAKER_02"])
    ...
    with (
        ...
        patch(
            "speechlib.speaker_recognition.speaker_recognition", return_value="unknown"
        ),
    ):
        result = detect_unknown_speakers(wav, "fake_voices", hf_token="tk")
